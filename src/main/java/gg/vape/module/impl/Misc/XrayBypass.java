...
public class XrayBypass extends Module {
    ...
    public SliderSetting range = new SliderSetting("Радиус", 20, 0, 100, 1).call(this);
    public SliderSetting height = new SliderSetting("Высота", 20, 0, 100, 1).call(this);
    public SliderSetting delay = new SliderSetting("Скорость", 7, 0, 10, 1).call(this);
    ...
    public void onReceivePacket(EventPacket e) {
        if (e.getPacket() instanceof SPacketBlockChange) {
            ...
            if (isEnabledOre(Block.getIdFromBlock(p.getBlockState().getBlock())) && !ores.contains(p.getBlockPosition())) {
                ...
                //Vape.notify.call("Xray", "Found ore at " + p.getBlockPosition().getX() + " " + p.getBlockPosition().getY() + " " + p.getBlockPosition().getZ(), NotificationType.INFO);
            }
            ...
            for (SPacketMultiBlockChange.BlockUpdateData dat : p.getChangedBlocks()) {
                if (isEnabledOre(Block.getIdFromBlock(dat.getBlockState().getBlock())) && !ores.contains(dat.getPos())) {
                        ores.add(dat.getPos());
                    //Vape.notify.call("Xray", "Found ore at " + dat.getPos().getX() + " " + dat.getPos().getY() + " " + dat.getPos().getZ(), NotificationType.INFO);
