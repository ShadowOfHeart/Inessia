...
public boolean drag;
public Module selMod = Medusa.m.m.get(0);
...
protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    ...
    int x = this.x + 40;
    int yMod = this.y + 50;
    for (Module m : Medusa.m.getModulesFromCategory(selected)) {
...
public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    drag(mouseX, mouseY);
    rotX = gg.vape.helpers.math.MathHelper.interpolate(drag ? (preX - mouseX) * 20 : 0, rotX, 0.01);
    GL11.glPushMatrix();
