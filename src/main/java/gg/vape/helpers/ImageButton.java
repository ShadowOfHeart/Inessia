...
public class ImageButton {
    ...
    public void onClick(int mouseX, int mouseY) {
        if (this.isHovered(mouseX, mouseY)) {
            ...
            if (this.target == 13) {
                FileHelper.showURL("http://vk.com/");
            }
            if (this.target == 12) {
                FileHelper.showURL("http://vk.com/");
            }
...
//            }
            ...
            if (this.target == 55) {
//                GuiCapeSelector.Selector.capeName = GuiCapeSelector.Selector.getCapeName().equals("lol1") ? "lol2" : "lol1";
            }
            if (this.target == 56) {
//                GuiCapeSelector.Selector.capeName = GuiCapeSelector.Selector.getCapeName().equals("lol2") ? "lol1" : "lol2";
            }
            if (this.target == 228) {
                FileHelper.showURL("http://vk.com/");
            }
    ...
    public void draw(int mouseX, int mouseY, Color color) {
        ...
        if (this.ani > 0.0f) {
            RenderUtil.drawImage(this.image, (float)this.x - this.ani, (float)this.y - this.ani, (float)(this.width + 4) + this.ani * 2.0f, (float)(this.height + 4) + this.ani * 2.0f, Color.BLACK);
            RenderUtil.drawImage(this.image, (float)this.x - this.ani, (float)this.y - this.ani, (float)this.width + this.ani * 2.0f, (float)this.height + this.ani * 2.0f, this.isHovered(mouseX, mouseY) ? new Color(156, 156, 156, 255) : Color.WHITE);
            if (this.isHovered(mouseX, mouseY)) {
                Fonts.BOLD16.drawStringWithShadow(this.description, (float)this.x + (float)this.width / 2.0f + (float)Fonts.BOLD16.getStringWidth(this.description) / 2.0f - (float)(this.target == 228 ? 35 : 6), this.y + this.height - 17, new Color(255, 255, 255, 255).getRGB());
            }
        } else {
            RenderUtil.drawImage(this.image, this.x, this.y, this.width + 3, this.height + 3, Color.BLACK);
            RenderUtil.drawImage(this.image, this.x, this.y, this.width, this.height, color);
        }
        GlStateManager.popMatrix();
    }

    protected boolean isHovered(int mouseX, int mouseY) {
        return HoverUtility.isHovered(this.x, this.y, this.x + this.width, this.y + this.height, mouseX, mouseY);
    }
}
