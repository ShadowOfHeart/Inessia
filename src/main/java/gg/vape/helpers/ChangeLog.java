...
import gg.vape.model.models.HashMapWithDefault;
...
public class ChangeLog {
    ...
    public void render() {
        float maxWidth = Fonts.BOLD14.getStringWidth(update.keySet().stream().max((s1, s2) -> Fonts.BOLD14.getStringWidth(s1) > Fonts.BOLD14.getStringWidth(s2) ? 1 : -1).get()) + 25;
        ...
        for (Map.Entry<String, Type> entry : update.entrySet()) {
            if (entry.getValue().name().equalsIgnoreCase("NEW")) {
                Fonts.BOLD14.drawString(ChatFormatting.GRAY + "[" + ChatFormatting.GREEN + "+" + ChatFormatting.GRAY + "] " + ChatFormatting.RESET + entry.getKey(), 10, y, -1);
            }
            else if (entry.getValue().name().equalsIgnoreCase("FIXED")) {
                Fonts.BOLD14.drawString(ChatFormatting.GRAY + "[" + ChatFormatting.YELLOW + "/" + ChatFormatting.GRAY + "] " + ChatFormatting.RESET + entry.getKey(), 10, y, -1);
            }
            else if (entry.getValue().name().equalsIgnoreCase("REMOVED")) {
                Fonts.BOLD14.drawString(ChatFormatting.GRAY + "[" + ChatFormatting.RED + "-" + ChatFormatting.GRAY + "] " + ChatFormatting.RESET + entry.getKey(), 10, y, -1);
            }
            y+=10;
        }
