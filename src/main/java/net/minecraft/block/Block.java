...
import gg.vape.module.impl.Movement.NoClip;
import gg.vape.module.impl.Player.Freecam;

import static gg.vape.helpers.render.RenderUtil.mc;
...
public class Block
{
    ...
    {
        if (mc.player != null) {
            if (Medusa.m.getModule(NoClip.class).state && MovementUtil.isMoving() && mc.player.collidedHorizontally)
                return NULL_AABB;
            if (Medusa.m.getModule(Freecam.class).state)
                return NULL_AABB;
        }
         else
