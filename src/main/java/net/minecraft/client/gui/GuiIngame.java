...
import gg.vape.module.impl.Render.NoScoreboard;
...
import gg.vape.module.impl.Player.Timer;
...
public class GuiIngame extends Gui
{
    ...
    {
        if (Medusa.protectedd)
            GL11.glScaled(1, 1 - Math.abs(Math.sin(System.nanoTime() / 1000000000f)), 1);
        animatedYText = gg.vape.helpers.math.MathHelper.interpolate(scrollPos, animatedYText, 0.05);
        ...
        Medusa.notify.render();
        {
            ScaleUtil.scale_pre();
            Timer t = (Timer) Medusa.m.getModule(Timer.class);
            if (t.smart.get()) {
                t.animWidth = (float) gg.vape.helpers.math.MathHelper.interpolate((100 - (t.ticks * 4)), t.animWidth, 0.05f);
...
//                RenderUtil.drawRectWH(ScaleUtil.calc(scaledresolution.getScaledWidth()) / 2f - 50, ScaleUtil.calc(scaledresolution.getScaledHeight()) - 70 + 1, t.animWidth, 5, ClickGui.color.get());
//                RenderUtil.drawBlurredShadow(ScaleUtil.calc(scaledresolution.getScaledWidth()) / 2f - 50, ScaleUtil.calc(scaledresolution.getScaledHeight()) - 70, t.animWidth, 5, 20, new Color(ClickGui.color.get()));
//                Fonts.RUB12.drawCenteredString((int) Math.round(t.animWidth) + "%", ScaleUtil.calc(scaledresolution.getScaledWidth()) / 2f, ScaleUtil.calc(scaledresolution.getScaledHeight()) - 67.5f, -1);
        ...
        a = gg.vape.helpers.math.MathHelper.interpolate((mc.gameSettings.keyBindPlayerList.isKeyDown() ? 1 : 0), a, 0.1);
    ...
    {
        if(Medusa.m.getModule(NoScoreboard.class).state){
            return;
        }
