...
import gg.vape.module.impl.Movement.NoPush;
import gg.vape.module.impl.Movement.Sprint;
import gg.vape.module.impl.Player.Freecam;
...
private float prePitch;
    private void onUpdateWalkingPlayer() {
        ...
        if (Medusa.m.getModule(Freecam.class).state) return;
...

    {
        ...
        for (Command command : Medusa.d.cmds) {
    ...
    {
        if (Medusa.m.getModule(NoPush.class).state) return false;
        if (Medusa.m.getModule(Freecam.class).state) return false;
        if (this.noClip)
        {
            return false;
        }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.onGround && !flag1 && !flag2 && this.movementInput.moveForward >= 0.8F && !this.isSprinting() && flag4 && !this.isHandActive() && !this.isPotionActive(MobEffects.BLINDNESS))
        {
            ...
            {
                if (Medusa.m.getModule(Sprint.class).state && ((Sprint) Medusa.m.getModule(Sprint.class)).allmode.get()) this.setSprinting(true);
