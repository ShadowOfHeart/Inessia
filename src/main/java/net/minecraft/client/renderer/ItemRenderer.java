...
import gg.vape.module.impl.Render.SwingAnimation;
...
import gg.vape.module.impl.Render.NoRender;
    ...
    public void renderItemInFirstPerson(AbstractClientPlayer player, float p_187457_2_, float p_187457_3_, EnumHand hand, float p_187457_5_, ItemStack stack, float p_187457_7_)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(hand))
        {
            ...
            else
            {
                ...
                {
                    float f3 = 1.0F - (this.prevEquippedProgressMainHand + (this.equippedProgressMainHand - this.prevEquippedProgressMainHand) * p_187457_2_);
                    ...
                    float f4 = this.mc.player.getSwingProgress(p_187457_2_);
                    ...
                    if(Medusa.m.getModule(SwingAnimation.class).state) {
                        if (enumhandside != (this.mc.gameSettings.mainHand.equals((Object) EnumHandSide.LEFT) ? EnumHandSide.RIGHT : EnumHandSide.LEFT)) {
                            ...
                            if (!mc.player.getHeldItemMainhand().isEmpty()) {
                                if(SwingAnimation.swordAnim.is("Tap")) {
                                    GlStateManager.translate(0.56F, -0.52F, -0.71999997F);
                                    GlStateManager.translate(0.0F, f3 * -0.15F, 0.0F);
                                ...
                                if(SwingAnimation.swordAnim.is("Nursultan")) {
                                    ...
                                    GlStateManager.rotate(this.mc.player.isSwingInProgress ? (float)(-alpha) / SwingAnimation.swingSpeed.get() : 0.0F, 1.0F, -0.0F, 1.0F);
                                    GlStateManager.translate(0.0, 0.0, 0.5);
                                }
                                if(SwingAnimation.swordAnim.is("Block")) {
...
//                            if (KseNonClient.m.getModule(ViewModel.class).state) {
//                                GlStateManager.scale(v.rightX.get(), v.rightY.get(), v.rightZ.get());
//                            }
                        } else {
...
//                    this.transformFirstPerson(enumhandside, p_187457_5_);
    ...
    public void renderOverlays(float partialTicks)
    {
        ...
        if (this.mc.player.isEntityInsideOpaqueBlock())
        {
            ...
            if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE)
            {
                ...
                {
                    if (!Medusa.m.getModule(NoRender.class).state && !((NoRender) Medusa.m.getModule(NoRender.class)).listSetting.selected.contains("Block")) {
    ...
    {
        if (Medusa.m.getModule(NoRender.class).state && ((NoRender) Medusa.m.getModule(NoRender.class)).listSetting.selected.contains("Fire")) return;
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
