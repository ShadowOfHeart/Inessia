...
{
    private float dragValue = (float) (ProtocolCollection.values().length - Arrays.asList(ProtocolCollection.values()).indexOf(ProtocolCollection.getProtocolCollectionById(viamcp.ViaMCP.PROTOCOL_VERSION))) / ProtocolCollection.values().length;
    ...
    {
        func_191745_a(mc, mouseX, mouseY, partialTicks);
    }
    ...
    protected void mouseDragged(Minecraft mc, int mouseX, int mouseY)
    {
        if (this.visible)
        {
            if (this.dragging)
            {
                ...
                this.displayString = values[(int) (this.sliderValue * (values.length - 1))].getVersion().getName();
                viamcp.ViaMCP.getInstance().setVersion(values[(int) (this.sliderValue * (values.length - 1))].getVersion().getVersion());
            }
