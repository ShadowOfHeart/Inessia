...
public class MCPViaPlatform implements ViaPlatform<UUID>
{
    ...
    {
        return String.valueOf(viamcp.ViaMCP.PROTOCOL_VERSION);
    }
    ...
    {
        return new FutureTaskId(CompletableFuture.runAsync(runnable, viamcp.ViaMCP.getInstance().getAsyncExecutor()).exceptionally(throwable ->
        {
    ...
    {
        return new FutureTaskId(viamcp.ViaMCP.getInstance().getEventLoop().submit(runnable).addListener(errorLogger()));
    }

    @Override
    public PlatformTask<?> runSync(Runnable runnable, long ticks)
    {
        return new FutureTaskId(viamcp.ViaMCP.getInstance().getEventLoop().schedule(() -> runSync(runnable), ticks * 50, TimeUnit.MILLISECONDS).addListener(errorLogger()));
    }

    @Override
    public PlatformTask<?> runRepeatingSync(Runnable runnable, long ticks)
    {
         return new FutureTaskId(viamcp.ViaMCP.getInstance().getEventLoop().scheduleAtFixedRate(() -> runSync(runnable), 0, ticks * 50, TimeUnit.MILLISECONDS).addListener(errorLogger()));
    }
